...
~~~
To write/put a new item into a DynamoDB table we send a PutCommand object to the DynamoDB engine. Update the stack (cdk deploy) and test the endpoint with Postman, using the JSON below as the body of the POST request:
~~~json
  {
    "movieId":614479,
    "genre_ids":[12,15,23],
...
~~~
The script generates JSON schema document from our TS type/interface definitions (shared/types.d.ts). Run the script:
~~~bash
$ npm run schema
~~~
