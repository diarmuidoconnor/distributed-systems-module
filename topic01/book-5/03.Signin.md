## SignIn/out Endpoints.

In `lib/auth-app-stack.ts` add signin and signout endpoint to the Auth web API:
...
~~~
Note, a JWT token is returned with a successful login attempt. This token must be used with subsequent requests as proof that the user has authenticated - we will tst this later.
...
~~~bash
$ git add -A
$ git commit -m "Confirm signsignin and signout endpoint"
~~~
