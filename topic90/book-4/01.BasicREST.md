...
$ cdk deploy
~~~
The above code's effect is to add a REST web API with two resources, both configured as a Lambda_Proxy integration. The two endpoints are:
...
The current lambda function behind the API's get-all-movies endpoint does not require modification; however, the get-a-specific-movie lamdba expects a movie id path parameter (as opposed to a query string parameter). In `lambdas/getMovieById.ts`:
...
Test both endpoints. For example, the get-a-movie response should look as follows:
