...
~~~
The lambda adds a new attribute to the table item for an image, e.g. Caption.
...
~~~
The filter used here is classified as a body filter. It examines a property(s) of the message body for certain valuesl in this case that the Records.s3.object.key has a value the starts with 'image'. Mseeage attributes are ignored for this filter. To test it, change the `message.json` to:
~~~json
 {  //  message.json
 name: 'image1.jpeg',
 value: 'Jane Smith'
